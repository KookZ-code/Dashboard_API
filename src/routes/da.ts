// DA Report endpoint — Die Attach area report, mirrors wb.ts structure.
// Endpoints: /api/v1/da/packages  and  /api/v1/da/report
...
interface ShiftEvent {
  ...
  t_start_fmt: string | null;
  t_end_fmt: string | null;
}
...
export default async function daRoutes(app: FastifyInstance): Promise<void> {
  ...
  // ── da/packages ─────────────────────────────────────────────────────────
  app.get<{ Querystring: { date: string } }>('/api/v1/da/packages',
    { preHandler: auth },
    async (req, reply) => {
    ...
    async (req, reply) => {
      ...
      for (const mid of target) {
        ...
        for (const ev of closedByMachine.get(mid) ?? []) {
          ...
          eventList.push({ job_type: ev.job_type ?? '', t_start: ev.t_start_fmt ?? '',
            t_end: ev.t_end_fmt ?? '', des_job: ev.des_job ?? '', dur_min: repair + wt, is_open: false });
        }
        ...
        for (const oj of openByMachine.get(mid) ?? []) {
          ...
          eventList.push({ job_type: oj.job_type, t_start: oj.t_start, t_end: '',
            des_job: oj.des_job ?? '', dur_min: oj.dur_min, is_open: true });
        }
        ...
        machineRows.push({ machine_id: mid, package: machinePkg.get(mid) ?? '',
          util_pct: round1(utilPct), wait_down_min: waitDown, down_min: downMin,
          wait_setup_min: waitSetup, setup_min: setupMin, setup_conv_min: setupConvMin,
          sbo_min: sboMin, total_loss_min: totalLoss, events: eventList });
      }
      ...
      const kpi = {
        total: n, n_down: machineRows.filter(r => r.down_min > 0).length,
